...
- An unselected model is a **`cli-default (identity unverified)`** — the runner does not know which model answered, and a `--resume` run passes nothing, so it inherits and verifies nothing.

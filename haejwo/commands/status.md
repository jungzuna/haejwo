...
4. **Observations** — read `state/observations.jsonl.1` (if present) then `state/observations.jsonl`, oldest-first; report the last ~10 lines from that combined order. Specifically report whether any record has a non-null `agent_type` (= hooks DO fire inside subagents and exemption is active) or all null (= hooks fire only for the main agent). This is the live evidence for the gate's subagent-exemption design.
